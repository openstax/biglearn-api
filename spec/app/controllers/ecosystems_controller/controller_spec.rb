...
RSpec.describe EcosystemsController, type: :request do
  ...
  let(:service_double)       do
    dbl = object_double(Services::CreateEcosystem::Service.new)
  ...
  context "when a valid request is made" do
    it "the request and response payloads are validated against their schemas" do
      expect_any_instance_of(EcosystemsController).to receive(:with_json_apis).and_call_original
    ...
    it "the response contains the course uuid returned by the CreateEcosystem service" do
      response_status, response_body = create_ecosystem(request_payload: request_payload)
      expect(response_body.fetch('created_ecosystem_uuid')).to eq(given_ecosystem_uuid)
    end
  end
end
