...
RSpec.describe Services::FetchAssignmentSpes::Service, type: :service do
  ...
  let(:requests_by_request_uuid)  { given_spe_requests.index_by { |req| req[:request_uuid] } }

  context "when non-existing AssignmentSpe assignment_uuids and algorithm_names are given" do
  ...
  context "when previously-existing AssignmentSpe assignment_uuids and algorithm_names are given" do
    ...
    it "the assignment_uuids are returned with exercise_uuids and 'assignment_ready'" do
      action.fetch(:spe_responses).each_with_index do |response, index|
        ...
        assignment_spe = AssignmentSpe.find_by assignment_uuid: assignment_uuid
        exercise_uuids = assignment_spe.exercise_uuids.first(request.fetch(:max_num_exercises))
